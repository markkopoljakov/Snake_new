import { PIXI, sprite } from 'expo-pixi';
...
class Board extends PIXI.Container {
  ...
  constructor(size, boardSize, interval) {
    ...
    PIXI.Sprite.from(hennoPilt).then(sprite => {
      this.henno = sprite;
      this.henno.height = this._tileSize;
      this.henno.width = this._tileSize;
      this.addChild(this.henno);
    });
    this.restart();
  }
  ...
  _generateFood = () => {
    const id = this.foodPositions[Math.floor(Math.random() * this.foodPositions.length)];
    ...
    PIXI.Sprite.from(ohvrid[Math.floor(Math.random() * ohvrid.length)]).then(sprite => {
      sprite.height = this._tileSize;
      sprite.width = this._tileSize;
      sprite.x = cell.transform.position._x;
      sprite.y = cell.transform.position._y;
  ...
  tick = interval => {
    ...
    this.secondsInterval = setInterval(() => {
      ...
      if(this.henno){
        this.henno.x = this.head.col*this._tileSize;
        this.henno.y = this.head.row*this._tileSize;
      }
      if (this.currentHead.isSnake) {
        if (this.currentHead.isFood) {
          this.snakeLength++;
          this.currentHead.isFood = false;
          if(this.sprites[this.currentHead.parentIndex]) {
            this.removeChild(this.sprites[this.currentHead.parentIndex]);
          }
          this._generateFood();
